...
async function sendInboxEmail({ subject, text, replyTo, headers = {} }) {
  ...
  if (provider === "resend") {
    ...
    return {
      ...result,
      provider: result.ok ? "resend" : result.provider,
    };
  }
  ...
  console.log("[inbox-mail]", payload);
...
export async function sendWaitlistEmail({ subject, text, signupId, requesterEmail }) {
  return sendInboxEmail({
    subject,
    text,
    replyTo: requesterEmail,
    headers: {
      "X-ShowFi-Waitlist-Signup": String(signupId || ""),
    },
  });
}
