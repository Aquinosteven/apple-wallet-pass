...
async function createAccountForOwner(supabase, user) {
  ...
  const fallback = await supabase
    .from("accounts")
    .select("id,owner_user_id,slug,name,billing_state,created_at,updated_at,is_primary_workspace")
    .eq("owner_user_id", user.id)
    .order("is_primary_workspace", { ascending: false })
...
export function computeBillingGateState({ account, subscription, now = new Date(), trialDays = 14 }) {
  ...
  const cancelRequestedAt = toIsoDate(metadata.cancel_requested_at);
  const currentPeriodEnd = toIsoDate(subscription?.current_period_end);
  const accessWindowActive = Boolean(
    currentPeriodEnd
      && new Date(currentPeriodEnd).getTime() > now.getTime()
  );
  const cancellationPending = cancelAtPeriodEnd && accessWindowActive;
  const cancellationEffective = cancelAtPeriodEnd && !accessWindowActive;

  const hasExplicitTrial = Boolean(toIsoDate(metadata.trial_ends_at));
  const trialEndsAt = hasExplicitTrial
    ? (toIsoDate(metadata.trial_ends_at) || trialEndsAtFromAccount(account, trialDays))
    : null;
  ...
  const subscriptionActive = isActiveSubscriptionStatus(subscriptionStatus)
    || cancellationPending
    || (subscriptionStatus === "canceled" && accessWindowActive);
  const accountAllowsAccess = accountBillingState === "active"
    && (!cancelAtPeriodEnd || accessWindowActive);

  const canAccessDashboard = subscriptionActive || accountAllowsAccess || trialActive;
  ...
  return {
    ...
    accessEndsAt: cancelAtPeriodEnd ? currentPeriodEnd : null,
    exitSurvey: metadata.exit_survey && typeof metadata.exit_survey === "object"
