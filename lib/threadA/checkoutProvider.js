...
function createSquareProvider(contract) {
  ...
  const paymentLinkConfigured = Boolean(
    contract.square.accessToken
    && contract.square.locationId
  );
  ...
  return {
    ...
    async createPayment({
      ...
      if (!planVariationId) {
        ...
        const paymentResponse = await squareRequest("/v2/payments", contract, {
          ...
          buyer_email_address: normalizeText(buyerEmailAddress) || undefined,
          note: `${String(planLabel || normalizedPlanCode || "ShowFi plan")} for ${String(accountId || "unknown_account")}`,
          autocomplete: true,
        });
        ...
        const payment = paymentResponse.payload?.payment || {};
        return {
          provider: "square",
          ...
          cardId: payment.card_details?.card?.id || null,
