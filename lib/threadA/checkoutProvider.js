...
async function squareRequest(path, contract, body) {
...
export function getCheckoutEnvContract(env = process.env) {
  return {
    provider: normalizeText(env.CHECKOUT_PROVIDER || env.BILLING_PROVIDER || "stub").toLowerCase(),
    square: {
      accessToken: normalizeText(env.SQUARE_ACCESS_TOKEN),
      locationId: normalizeText(env.SQUARE_LOCATION_ID),
      environment: normalizeText(env.SQUARE_ENVIRONMENT || "sandbox").toLowerCase(),
      webhookSignatureKey: normalizeText(env.SQUARE_WEBHOOK_SIGNATURE_KEY),
    },
  };
}
...
function createSquareProvider(contract) {
  ...
  return {
    provider: "square",
    async createCheckoutSession({ accountId, planCode }) {
      if (!configured) {
        return {
          provider: "square",
          checkoutUrl: null,
      ...
      const requestBody = {
        ...
        quick_pay: {
          name: String(planCode || "showfi_plan"),
          price_money: {
            amount: 9900,
            currency: "USD",
          },
    ...
    async parseWebhook(input = {}) {
      ...
      return {
        ...
        providerSubscriptionId: data?.subscription?.id || data?.id || null,
        providerCustomerId: data?.subscription?.customer_id || data?.customer_id || null,
