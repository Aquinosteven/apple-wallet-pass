...
create table if not exists public.account_usage_monthly (
  ...
  issuances_count integer not null default 0 check (issuances_count >= 0),
...
create table if not exists public.webhook_endpoints (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check (event_id is null or account_id is not null)
);
...
create index if not exists account_usage_monthly_account_month_idx
on public.account_usage_monthly (account_id, usage_month);
...
create index if not exists webhook_dead_letter_account_created_idx
on public.webhook_dead_letter (account_id, created_at desc);
...
drop trigger if exists account_usage_monthly_set_updated_at on public.account_usage_monthly;
create trigger account_usage_monthly_set_updated_at
before update on public.account_usage_monthly
for each row execute function public.set_updated_at();
